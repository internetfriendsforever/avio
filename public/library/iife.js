var avio = (function (exports, kefir, wobble) {
  'use strict';

  function ensureKefirStream (any) {
    return typeof any === 'number' ? kefir.constant(any) : any.kefirProperty
  }
  ...
  function create (kefirStream, initial = 0) {
    ...
    decorate(stream, 'spring', function (options) {
      ...
      const spring = new wobble.Spring(options);
  ...
  const raf = kefir.stream(emitter => {
  ...
  var mouse = {
    ...
    down: create((
      kefir.merge([
        kefir.fromEvents(window, 'mousedown').map(() => 1),
        kefir.fromEvents(window, 'mouseup').map(() => 0)
      ])
    ), 0),

    up: create((
      kefir.merge([
        kefir.fromEvents(window, 'mousedown').map(() => 0),
        kefir.fromEvents(window, 'mouseup').map(() => 1)
      ])
    ), 0)
  };
  ...
  var touch = {
    down: create((
      kefir.merge([
        kefir.fromEvents(document, 'touchstart').map(() => 1),
        kefir.fromEvents(document, 'touchend').map(() => 0)
      ])
    ), 0),

    x: create(
      kefir.merge([
        kefir.fromEvents(document, 'touchmove').map(e => e.changedTouches[0].clientX / window.innerWidth),
        kefir.fromEvents(document, 'touchstart').map(e => e.changedTouches[0].clientX / window.innerWidth)
      ])
    ),

    y: create(
      kefir.merge([
        kefir.fromEvents(document, 'touchmove').map(e => e.changedTouches[0].clientY / window.innerHeight),
        kefir.fromEvents(document, 'touchstart').map(e => e.changedTouches[0].clientY / window.innerHeight)
      ])
    )
  };
  ...
  const resize = kefir.fromEvents(window, 'resize').map(getSize).toProperty(getSize);
  ...
  function add (instance) {
    ...
    renderStream = raf$1(kefir.combine([resize, ...kefirProperties], (size, ...properties) => (
      properties
    ))).onValue(render);
  ...
  function shape ({ type, props, render }) {
    return function () {
      ...
      const kefirProperty = kefir.stream(e => {
        emitter = e;
